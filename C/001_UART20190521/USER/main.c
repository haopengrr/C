...
//USART中断设置
	USART_ITConfig(USART1,USART_IT_RXNE,ENABLE);	//读数据寄存器非空RXNE。当RDR移位寄存器中的数据被转移到USART_DR寄存器中，该位被硬件置位。如果USART_CR1寄存器中的RXNEIE为1，则产生中断。
...
//中断服务函数,USART1_IRQHandler是一个特定的中断函数，在startup_stm32f10x_hd.s中
